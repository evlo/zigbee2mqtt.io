...
TIPs: Do NOT have any other zigbee coordinator then the one you are pairing device with (ie. not even ikea gw) turned on, does not matter if it is in pairing mode or not. If the device does not start pairing within 20 seconds of fast 4 button presses just do the presses again. If the device does have issue finishing iterview mode keeep pressing pairing button in about 2 secondis interval during interview.
...
Battery charge indicataor on this unit can be a bit trusted after about 1 day device is on the network, device falsely reports low battery state after pairing.
